...
def caregiver_dashboard():
    ...
    for p in patients:
        # Get today's log
        log = conn.execute('SELECT * FROM medication_logs WHERE patient_id = ? AND date = ?', 
                           (p['id'], today)).fetchone()
...
def get_patient_logs(patient_id):
    ...
    conn = get_db_connection()
    logs = conn.execute('SELECT * FROM medication_logs WHERE patient_id = ?', (patient_id,)).fetchall()
    conn.close()
